(function(){
    ...
    angular
        .module('lotes.lote')
        .controller('ProcesosGenerarDocsCorreoController', ProcesosGenerarDocsCorreoController);
        
        ProcesosGenerarDocsCorreoController.$inject = ['$scope', '$state', 'moment', '$filter', '$log', '$document', '$http', '$q', '$timeout', '$mdDialog', 'procesosService','PROCESOS'];
        function ProcesosGenerarDocsCorreoController($scope, $state, moment, $filter, $log, $document, $http, $q, $timeout, $mdDialog, procesosService, PROCESOS) {
                ...
                $scope.datos_csv = {};
                $scope.datos_csv.es_archivo_respuesta = false;
                $scope.datos_csv.track_trace_modelo = 'CU000000000CR';
                $scope.data = {
                    modo_carga: {},
                    id_remito: null
                };
               ...
               activate();
                ...
                $scope.generarDocsCorreo = generarDocsCorreo;
                ...
                function generarDocsCorreo( event ){
                    ...
                    console.log(pkg_imposicion.data)

                    for(var index in pkg_imposicion.data){
                        //console.log(pkg_imposicion.data[index])
                        numero_pieza = pkg_imposicion.data[index][0];
                        track_trace = pkg_imposicion.data[index][1];
                        str_imposicion += numero_pieza + sep_cmp_imp + track_trace + sep_cmp_imp + 'PIEZA RECIBIDA' + sep_cmp_imp;
                        imposiciones.push(str_imposicion);
                        str_imposicion = '';
                    }


                    //imposiciones = imposiciones.join("#");
                    //imposiciones = imposiciones.replace(/#/gi, '\r\n');
                    console.log(imposiciones)
                    procesosService
                        .generarImposicion( imposiciones )
                        .then (
                ...
                function leerCSV( contents ){
                    ...
                    /*if($scope.datos_csv.es_archivo_respuesta){
                        $scope.datos_csv.CSV_content[$scope.datos_csv.registros_totales-1].split(';')[0]

                        $scope.datos_csv.registros_totales = ($scope.datos_csv.CSV_content.length)-1;
                    }else{
                        $scope.datos_csv.registros_totales = ($scope.datos_csv.CSV_content.length)-1;
                    }
                    $scope.datos_csv.registros_totales = ($scope.datos_csv.CSV_content.length)-1;
                ...
                function armarPaqueteDatos(  ){

                    var data_body = [];
                    var acta = [];
                    var codigos_barras = [];
                    var numero_acta, codigo_barras, notificar;
                    var separador_campos, codigo_respuesta, numero_pieza, track_trace, registro_imp = '';
                    var cant_campos, cant_registros = 0
                    var codigo_respuesta_exito = "00";
                    var campos_imposicion = {
                    ...
                    //cant_campos = $scope.datos_csv.CSV_content[0];
                    ...
                    for(var index in $scope.datos_csv.CSV_content){
                        ...
                        if($scope.datos_csv.es_archivo_respuesta){
                            ...
                            separador_campos = ';';
                            cant_campos = ($scope.datos_csv.CSV_content[index].split(separador_campos).length);
                            console.log(cant_campos)
                            if(cant_campos  > 96){
                                codigo_respuesta = $scope.datos_csv.CSV_content[index].split(separador_campos)[cant_campos-1];                            
                                console.log(codigo_respuesta)
                                if(codigo_respuesta.indexOf(codigo_respuesta_exito) > -1){
                                    console.log($scope.datos_csv.CSV_content[index].split(separador_campos))
                                    cant_registros ++;
                                    numero_pieza = $scope.datos_csv.CSV_content[index].split(separador_campos)[campos_imposicion.numero_pieza];
                                    track_trace = $scope.datos_csv.track_trace_modelo.substring(0,8) + lPad(String(cant_registros)) + $scope.datos_csv.track_trace_modelo.substring(11)//'CU000000000AR';
                                    data_body.push([numero_pieza,track_trace]);
                                }
                            }
                    ...
                    $scope.datos_csv.cant_registros = cant_registros;

                    pkg_imposicion.header = cant_registros;
                    pkg_imposicion.data = data_body;
