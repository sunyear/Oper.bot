(function(){
    ...
    angular
        ...
        LoteController.$inject = ['$scope', '$state', 'moment', '$filter', '$log', '$document', '$http', '$q', '$timeout', '$mdSidenav', 'loteService','PROCESOS'];
        function LoteController($scope, $state, moment, $filter, $log, $document, $http, $q, $timeout, $mdSidenav, loteService, PROCESOS) {
                ...
                vm.nro_lote, vm.nro_lote_imp = 0;

                vm.CSVContent, vm.CVSImpContent = '';
                vm.CSVFilename, vm.CSVImpFilename =  '';


                vm.registrosTotales, vm.registrosTotalesImposicion = 0;

                vm.registrosErrores = 0;
                vm.registrosErroresImposicion = [];
                vm.errores_imposicion_cuenta = 0;

                vm.registrosImpDuplicadosTotales = 0;
                ...
                //CAMPOS DEL CSV DE IMPOSICION Y SUS POSICIONES
                var POS_NUMERO_PIEZA = 0, // NUMERO DE PIEZA
                    POS_TRACK_AND_TRACE = 1,   //TRACK AND TRACE DE LA PIEZA
                    POS_LEYENDA_PIEZA = 29;  //"PIEZA RECIBIDA"/"PIEZA FALTANTE"/"PIEZA SOBRANTE"
                ...
                $scope.leerCSVImposicion = leerCSVImposicion;

                vm.procesarLote = procesarLote;
                vm.procesarLoteImposicion = procesarLoteImposicion;
                ...
                //vm.generaCodigoBarras56 = generaCodigoBarras56;
                //vm.generarActas = generarActas;
                //vm.cargarParams = cargarParams;
                //vm.generarDigitoVerificador54 = generarDigitoVerificador54;
                //vm.generarDigitoVerificador56 = generarDigitoVerificador56;
                //metodos auxiliares
                ...
                function leerCSVImposicion( contents ){

                        vm.CSVImpFilename = contents[0];
                        vm.CSVImpContent = contents[1];

                        //var regex = new RegExp("[0-9]{8}(?=_)", "g");
                        var log = vm.CSVImpFilename.match("[0-9]{8}(?=_)");
                        vm.nro_lote_imp = log[0].substring(-8);

                        //console.log();
                        
                        var aFile = vm.CSVImpContent.split('\n');
                        vm.registrosTotalesImposicion = (aFile.length) -1;

                        buscarDuplicados(aFile);
                        buscarRechazos(aFile);

                        aFile = '';//dummy

                        vm.btnProcesarDisabled = false;
                        return ( vm.CSVImpContent );
                };
                ...
                function procesarLoteImposicion(){

                        var cRegistros = 10;
                        
                        //Genero un array; cada elemento corresponde a un registro del archivo CSV
                        aFile = vm.CSVImpContent.split('\n');
                        cRegistros = (aFile.length) -1;
                        vm.registrosTotalesImposicion  = cRegistros;

                        //vm.exito = buscarDuplicados(aFile);
                        //vm.exito = buscar

                        /*
                        for(var i=0; i<cRegistros; i++){
                                vm.exito = extraerCamposImposicion(aFile[i]);
                        };
                        */
                ...
                function extraerCamposImposicion( registro ){

                    var exito = true;


                    var aRegistro = registro.split('|');
                    var id_imposicion = aRegistro[0];
                    var numero_pieza = aRegistro[POS_NUMERO_PIEZA],
                        track_and_trace = aRegistro[POS_TRACK_AND_TRACE],
                        leyenda_pieza = aRegistro[POS_LEYENDA_PIEZA];
                ...
                function buscarDuplicados( archivoImposicion ){

                    var arrNroPiezas = [];
                    var arrPiezasDuplicadas = [];

                    function contarDuplicados( numero_pieza_ctrl ){
                       var c = 0;
                       var arr = [];

                        for(var i in arrNroPiezas){
                            //console.log(numero_pieza)
                            if(arrNroPiezas[i] == numero_pieza_ctrl){

                                arr.push(numero_pieza_ctrl)
                            }
                        }
                    ...
                    angular.forEach(archivoImposicion, 
                            function(value, key) {
                                var tmp = value.split('|');
                                var numero_pieza = tmp[POS_NUMERO_PIEZA];
                                arrNroPiezas.push( numero_pieza);
                                var pieza_duplicada = contarDuplicados(numero_pieza);
                                if(pieza_duplicada){
                                    arrPiezasDuplicadas.push(numero_pieza);
                                }
                            }
                    );

                    //console.log(arrPiezasDuplicadas)
                    vm.registrosImpDuplicadosTotales = arrPiezasDuplicadas.length;

                }


                function buscarRechazos( archivoImposicion ){

                    //var arrRechazos = [];

                    angular.forEach(archivoImposicion,

                            function explorar(registro, clave){
                                if(registro.length > 0){
                                    var campos = registro.split('|');
                                    var cod_error = campos[campos.length -1].trim();
                                    if(cod_error != '00'){
                                        vm.registrosErroresImposicion.push(registro);
                                    }
                                }
                            }
                    ...
                    vm.errores_imposicion_cuenta = vm.registrosErroresImposicion.length;
